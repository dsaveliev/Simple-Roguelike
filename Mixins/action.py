...
def apply(item, owner, action, supply=[]):
  ...
  if action == 'QUAFF':
    Text.event_quaff(item.world.panel, owner.name, item.name)
    quaff(item, owner)
  elif action == 'EAT':
    Text.event_eat(item.world.panel, owner.name, item.name)
    eat(item, owner)
  elif action == 'READ':
    Text.event_read(item.world.panel, owner.name, item.name)
    read(item, owner)
  elif action == 'THROW':
    throw(item, owner)
  elif action == 'EQUIP':
