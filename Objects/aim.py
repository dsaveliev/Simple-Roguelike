...
class Aim(Object):
  ...
  def move_or_attack(self, dx, dy):
    ...
    if x >= 0 and y >= 0 and x < MAP['WIDTH'] and y < MAP['HEIGHT']:
      self.x = x
      self.y = y
      names = self.world.get_names_at_position(self.x, self.y)
      Text.event_observation(self.world.panel, names)

  def aim(self):
    self.world.game_state = 'PLAYING'
    Object.list.remove(self)
    targets = Creature.get_by_position(self.x, self.y)
    for target in targets:
      if libtcod.map_is_in_fov(self.world.map.fov_map, target.x, target.y):
