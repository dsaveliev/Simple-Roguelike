...
class PlainText(object):
  ...
  def show(self, position = (None, None, None, None)):
    window = libtcod.console_new(position[2], position[3])
    libtcod.console_set_default_foreground(window, libtcod.lighter_sepia)

    current_line = 0
    for line in open(self.file,'r'):
      libtcod.console_print(window, 1, 1 + current_line, line)
      current_line += 1
    
    libtcod.console_blit(window, 0, 0, position[2], position[3], 
